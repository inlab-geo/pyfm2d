...
myfmm.fmm.set_solver_options(np.int32(dicex),
                             np.int32(dicey),
...
gdx, gdz, asgr, sgdl, sgs, earth, fom, snb, fsrt, cfd, wttf, wrgf = myfmm.fmm.get_solver_options()

print('Original options: ' ,dicex,dicey,int(sourcegridrefine),sourcedicelevel,sourcegridsize,earthradius,schemeorder,nbsize,lttimes,lfrechet,tfieldsource+1,lpaths)
print('Recovered options:',gdx, gdz, asgr, sgdl, sgs, earth, fom, snb, fsrt, cfd, wttf, wrgf)

# test velocity model input

nvx,nvy,dlat,dlong,vc,noncushion,nodemap = build_velocitygrid(v,extent) # add cushion layer to velocity model and get parameters
