...
#
def calcjacobian(kernel,basis,mthd='simpson',epsrel=1.0E-3,returnerror=False,forceint=False,verbose=False,csc=False):
    ...
    if(not csc):
        ...
        if(mthd=='quad'): JacobianE = np.zeros([ndata,nbasis]) 
    ...
    if(csc): # output Jacobian in csc format
        ...
        if(returnerror): JdataE = np.zeros(ndata*nbasis)
        for j in tqdm(range(nbasis)): # loop over data
            indptr[j]=k
            for i in range(ndata):
                if(verbose): print(' Kernel function '+str(i)+' basis function '+str(j))
                if(kernel.type=='1Dstraightray' and (basis.type == '3Dvoxel' or basis.type == '2Dpixel') and not forceint):
                    r = raylengths(i,j,kernel,basis) # special case for ray lengths in voxel cells.  
                    if(r != 0.0):
                        Jindices[k] = i 
                    ...
                    a,b = integrate_kernelandbasis(i,j,kernel,basis,epsrel=epsrel,mthd='quad')
                    if(a != 0.0):
                        Jindices[k] = i 
                        Jdata[k] = a
                        JdataE[k] = b
                    ...
                    if(r != 0.0):
                        ...
                        k+=1
        indptr[nbasis]=k
        Jacobian = scipy.sparse.csc_matrix((Jdata[:k], Jindices[:k], indptr), shape=(ndata, nbasis))
        if(returnerror): Jacobian,scipy.sparse.csc_matrix((JdataE[:k], Jindices[:k], indptr), shape=(ndata, nbasis))
        ...
        for i in tqdm(range(ndata)): # loop over data
            for j in range(nbasis):
                ...
                if(kernel.type=='1Dstraightray' and (basis.type == '3Dvoxel' or basis.type == '2Dpixel') and not forceint):
                    ...
                    Jacobian[i,j],JacobianE[i,j] = integrate_kernelandbasis(i,j,kernel,basis,epsrel=epsrel,mthd='quad')
                elif(mthd=='simpson'): 
                    Jacobian[i,j] = integrate_kernelandbasis(i,j,kernel,basis,mthd='simpson')
    if(returnerror): return Jacobian,JacobianE   # return Jacobian and estimated numerical error
    return Jacobian   # return Jacobian and estimated numerical error


def integrate_kernelandbasis(i,j,kernel,basis,epsrel=1.0E-3,mthd='simpson'): # Adaptive numerical integration
    
    if(kernel.type=='1Dstraightray' or kernel.type == '1Dcurveray'):
        if(mthd=='simpson'): return simps1D(i,j,kernel,basis)
        if(mthd=='quad'): return quad1D(i,j,kernel,basis,epsrel=epsrel)
    elif(kernel.type=='2D'):
        if(mthd=='quad'): return quad2D(i,j,kernel,basis,epsrel=epsrel)
        if(mthd=='simpson'): return simps2D(i,j,kernel,basis)
    elif(kernel.type=='3D'):
        if(mthd=='quad'): return quad3D(i,j,kernel,basis,epsrel=epsrel)
        if(mthd=='simpson'): return simps3D(i,j,kernel,basis)

def quad1D(i,j,kernel,basis,epsrel=1.0E-3): # Adaptive numerical integration along a ray
    f = lambda x: integrand1D(i,j,kernel,basis,x)
    return quad(f, 0., kernel.lengths[i],epsrel=epsrel)
...
def quad2D(i,j,kernel,basis,epsrel=1.0E-3):
    ...
    return integrate.dblquad(f, xlim[0], xlim[1], lambda x: zlim[0], lambda x: zlim[1],epsrel=epsrel)

def quad3D(i,j,kernel,basis,epsrel=1.0E-3):
    f = lambda z, y, x: integrand3D(i,j,kernel,basis,(x,y,z))
    xlim,ylim,zlim = basis.intlimits(j) # set limits of integration for this basis function
    return integrate.tplquad(f, xlim[0], xlim[1], 
                      lambda x: ylim[0], lambda x: ylim[1],
                      lambda x, y: zlim[0], lambda x, y: zlim[1],epsrel=epsrel)

def simps1D(i,j,kernel,basis): # Simpsons rule integration for 1D kernel 
    l = np.linspace(0., kernel.lengths[i],basis.nline)
...
def integrand3D(i,j,kernel,basis,pos): # evaluates the integrand for use by integration routines
    if(basis.type=='3Dvoxel' or basis.type=='2Dpixel'): # if voxel basis it is assumed that x,y,z lies within the jth cell where basis value is one.
        return kernel.evaluate(i,pos)
    else:
        return kernel.evaluate(i,pos)*basis.evaluate(j,pos)
    
# Integrand for a 2D kernel in a 2D model
def integrand2D(i,j,kernel,basis,x,y): # evaluates the integrand for use by integration routines
    if(basis.type=='3Dvoxel'): # if voxel basis it is assumed that x,y,z lies within the jth cell where basis value is one.
...
def integrand1D(i,j,kernel,basis,l): # evaluates the integrand for use by integration routines
    #x,y,z = kernel.position(i,l)
    pos = kernel.position(i,l)
    return basis.evaluate(j,pos)   

def checkinsidecell(x,xlim,ylim,zlim): # check if point is inside cell (not on boundary)
    if(x[0] <= xlim[0]): return False
    if(x[0] >= xlim[1]): return False
    if(x[1] <= ylim[0]): return False
    if(x[1] >= ylim[1]): return False
    if(x[2] <= zlim[0]): return False
    if(x[2] >= zlim[1]): return False
    return True    
    
 # Calculate ray lengths in 2D pixel or 3D voxel model
def raylengths(i,j,kernel,basis):       # Calculates lengths of rays in cellular grid in 2D or 3D
    ...
    aabb = np.array([[xlim[0],ylim[0],zlim[0]],[xlim[1],ylim[1],zlim[1]]]) # cell defined by extremes
    ...
    if(p0 is None): # line does not intersect cell
        ...
        if(checkinsidecell(raystart,xlim,ylim,zlim)): 
            if(checkinsidecell(rayend,xlim,ylim,zlim)): return np.linalg.norm(raystart-rayend) # both end points are inside cell return raylength
            return np.linalg.norm(raystart-p0) # if start point is in cell return raylength  
        if(checkinsidecell(rayend,xlim,ylim,zlim)): return np.linalg.norm(rayend-p0) # if end point is in cell return raylength  
...
def calcjacobian_nonsparse(kernel,basis,mthd='simpson',epsrel=1.0E-3,returnerror=False,forceint=False,verbose=False):
