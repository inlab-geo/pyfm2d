...
class pixelbasis2D(object): # Model basis class
    ...
    """
    def __init__(self,xg,zg,npoints=[50,50,1000],outer='x'): # receives linear grid dimension
        self.type = '2Dpixel'
        self.name = '2Dpixel'
    ...
    def evaluate(self,j,pos): # evaluate the ith model basis function at location (x,y)
        if (j < 0 or j >= self.nbases): raise BasisError(j)
        x,z = pos
        ...
        if(np.isscalar(x)):
            ...
            if((ix==self.nx-1 and x == self.xg[-1]) or (iz==self.nz-1 and z == self.zg[-1])): b = 1. # Correct upper boundary
        else:
            b = np.zeros_like(x)
    ...
    def convertindex(self,i):   # Convert single 1D array index to 2D array index using inner loop over X and outer loop over Y
        if (i < 0 or i >= self.nbases): raise BasisError(i)
        if(self.outer == 'x'):
    ...
    def iconvertindex(self,ix,iz):   # Convert single 2D array index to 1D array index inner loop over X and outer loop over Y
        if (ix < 0 or ix >= self.nx): raise BasisError(ix)
        if (iz < 0 or iz >= self.nz): raise BasisError(iz)
        if(self.outer == 'x'):
    ...
    def intlimits(self,j):      # calculates the limits of integration for given basis function
        ix,iz = self.convertindex(j)               # convert j basis to 2D model basis index
...
class cosinebasis2D(object): # Model basis class
    ...
    """
    def __init__(self,x0,x1,z0,z1,nx,ny,npoints=[120,120,200]): # receives linear grid dimension
        self.type = '2D'
        self.name = '2Dcosine'
    ...
    def fitcoefficients(self,target,A=None,returnmatrix=False): 
        ...
        coeff,res,rank,s = np.linalg.lstsq(A.T, target.flatten(),rcond=None) # fit basis coefficients to input field
        if(returnmatrix): return coeff.reshape(self.nx,self.nz),A
        return coeff.reshape(self.nx,self.nz)

    def getbasisImage(self,j,nx=None,nz=None): # returns 2D image of model at chosen resolution
        if(nx==None): nx = self.nx
        if(nz==None): nz = self.nz
        dx, dz = (self.xmax - self.xmin)/nx, (self.zmax - self.zmin)/nz
        Zm,Xm = np.meshgrid(np.linspace(self.zmin+dz/2,self.zmax-dz/2,nz),np.linspace(self.xmin+dx/2.,self.xmax-dx/2,nx))
...
class raykernel1D(object): # A linear data kernel class
    ...
    def __init__(self,paths): # receives the end points of rays.
        ...
        if(type(paths)==list):
            ...
            for k,p in enumerate(paths):
                ...
                dcumsum.append(np.array([0.0,*np.cumsum(dls)]))  # cummulative lengths of each ray segment
    ...
    def evaluate(self,i): # evaluate the ith data kernel at location (x,y,z)
        if (i < 0 or i >= self.nkernels): raise KernelOBSerror(i) 
            
        return self.constant # Return constant along ray for integration       

    def position(self,i,l): # find (x,y,z) position of ith data kernel at length l along ray. 
        #
        ...
        if(self.type == '1Dstraightray'):
            ...
            j = np.clip(np.searchsorted(self.cumlengths[i], l),1,len(self.cumlengths[i])-1)
            ...
            a1 = self.paths[i][j]
            alpha = (l-d0)/dl            
            out = (1.0-alpha[:,np.newaxis])*a0 + alpha[:,np.newaxis]*a1 # interpolate along segment for location
            return out.T
...
def randomrays2Dbox(Nrays,x,y,seed=61254557,filename=''): # Generate random rays across 2D model
...
def _choose_endpoint(side,x,y): # choose random point on boundary of 2D box
    if(side == 0):
        return [x[0],y[0] + (y[-1]-y[0])*np.random.random()] # choose endpoint randomly along xmin side
    elif(side == 1):
        return [x[-1],y[0] + (y[-1]-y[0])*np.random.random()] # choose endpoint randomly along xmax side
    elif(side == 2):
        return [x[0] + (x[-1]-x[0])*np.random.random(),y[0]] # choose endpoint randomly along ymin side
    else:
        return [x[0] + (x[-1]-x[0])*np.random.random(),y[-1]] # choose endpoint randomly along ymax side
